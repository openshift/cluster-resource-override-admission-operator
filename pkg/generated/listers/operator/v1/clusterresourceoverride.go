...
type ClusterResourceOverrideLister interface {
	...
	List(selector labels.Selector) (ret []*operatorv1.ClusterResourceOverride, err error)
	// Get retrieves the ClusterResourceOverride from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*operatorv1.ClusterResourceOverride, error)
...
type clusterResourceOverrideLister struct {
	listers.ResourceIndexer[*operatorv1.ClusterResourceOverride]
}
...
func NewClusterResourceOverrideLister(indexer cache.Indexer) ClusterResourceOverrideLister {
	return &clusterResourceOverrideLister{listers.New[*operatorv1.ClusterResourceOverride](indexer, operatorv1.Resource("clusterresourceoverride"))}
}
