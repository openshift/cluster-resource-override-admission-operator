...
func (c *certGenerationHandler) Handle(context *ReconcileRequestContext, original *selinuxfixv1.SelinuxFixOverride) (current *selinuxfixv1.SelinuxFixOverride, result controllerreconciler.Result, handleErr error) {
	current = original
	ensure := false
	...
	if secretGetErr != nil && !k8serrors.IsNotFound(secretGetErr) {
		handleErr = condition.NewInstallReadinessError(selinuxfixv1.InternalError, secretGetErr)
		return
	}
	...
	if configMapGetErr != nil && !k8serrors.IsNotFound(configMapGetErr) {
		handleErr = condition.NewInstallReadinessError(selinuxfixv1.InternalError, configMapGetErr)
		return
	}
	...
	if ensure {
		...
		if err != nil {
			handleErr = condition.NewInstallReadinessError(selinuxfixv1.CannotGenerateCert, err)
			return
		}
