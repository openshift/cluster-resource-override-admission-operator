...
func (m *mutatingWebhookConfiguration) Name() string {
	return fmt.Sprintf("%s.%s", APIGroupResource, APIGroup)
}

func (m *mutatingWebhookConfiguration) New() *admissionregistrationv1.MutatingWebhookConfiguration {
	url := fmt.Sprintf("https://localhost:9401/apis/%s/%s/%s", APIGroup, APIGroupVersion, APIGroupResource)
	policy := admissionregistrationv1.Fail
	matchPolicy := admissionregistrationv1.Equivalent
	namespaceMatchLabelKey := fmt.Sprintf("%s.%s/enabled", APIGroupResource, APIGroup)
