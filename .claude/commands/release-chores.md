...
2. `$2` - **new_go_version** (optional): New Go version (e.g., `1.24`) - auto-detected from `.ci-operator.yaml` if not provided
...
- **Go version**: The Go version in `go.mod` MUST match what's in `.ci-operator.yaml`
...
2. Validate inputs:
   ...
   - If `NEW_GO_VERSION_OVERRIDE` is provided, ensure it matches format `X.Y` (e.g., `1.24`)
...
3. If version matches, extract the Go version from `.ci-operator.yaml` - this is the authoritative Go version to use
...
Go:                 [DOCKERFILE_GO_VERSION] → [NEW_GO_VERSION] (from .ci-operator.yaml) [GO_IMAGE_STATUS]
...
3. **Update Go version** in `@images/ci/Dockerfile`:
   - Replace `$DOCKERFILE_GO_VERSION` with `$NEW_GO_VERSION` (from `.ci-operator.yaml`)
   - Only if they differ
...
7. **Update go.mod Go version**:
   - Extract current Go version from `go.mod` using pattern `^go \K[0-9]+\.[0-9]+`
   - If it differs from `$NEW_GO_VERSION`, update it to `go $NEW_GO_VERSION.0`
   - Also update `toolchain` directive if present: `toolchain go$NEW_GO_VERSION.0`
...
2. Verify go.mod matches .ci-operator.yaml:
   ```bash
   grep -E '^go ' go.mod && grep golang .ci-operator.yaml
   ```
...
- Update Go version from $DOCKERFILE_GO_VERSION to $NEW_GO_VERSION
...
- If `.ci-operator.yaml` bypass was used, add a warning section at the top
